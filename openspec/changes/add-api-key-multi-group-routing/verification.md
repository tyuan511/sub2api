...
### 2026-09-06 审查问题修复（本地）

- `codex/smart-routing` 已合并当前 `main`（提交 `8e88caced`），完整 `go test -tags unit ./... -count=1` 通过；前端直接执行 `vue-tsc -b`、Vite 生产构建和 35 项路由/颜色/排序回归通过。
- 评分后台只有存在未删除、active 且至少两个启用候选的 Key 才会运行；健康、价格和失败聚合也按同一集合过滤，不再扫描单组用户的全量 `usage_logs`。迁移 251 通过 `CREATE INDEX CONCURRENTLY` 建立激活集合 partial index，并覆盖 invalid index 重试。
- 迁移 240/241/248 的线上安全改动使用 `NOT VALID`，usage decision 索引使用迁移 250 的并发创建；只读生产预检脚本已覆盖 240–251、锁等待、索引进度、回填和约束验证状态。hk1/us02 预检尚未在本轮执行，不能据此标记线上已验证。
- 单分组配置保存保持等价时不递增路由版本；跨组故障转移状态重建共享本请求尝试预算；旧 `ttft_p50_ms`/`duration_p50_ms` 仅作为兼容输入，评分使用明确的平均值字段。
...
- 本文件第 4 节全部有 hk1 证据；
- 灰度阶段无未解释的硬护栏越界；
- 回滚演练成功，将受影响 Key 改回单组后旧 `group_id` 镜像仍可服务；
- 告警、运行手册和用户说明已随发布交付；
- 生产扩大流量另行获得明确授权。
