...
默认地址：前端 `http://localhost:3000`，后端 `http://127.0.0.1:8080`。默认管理员为 `admin@sub2api.local / admin123456`，首次运行后请及时修改密码。数据库容器会持续运行，按 `Ctrl-C` 只停止本地前后端。

如端口冲突，可在运行前设置 `DATABASE_HOST_PORT`、`REDIS_HOST_PORT`、`SERVER_PORT`、`FRONTEND_PORT` 或 `ADMIN_PASSWORD`。
