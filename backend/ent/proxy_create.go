...
func (_c *ProxyCreate) AddAccountProxyPoolIDs(ids ...int64) *ProxyCreate {
	_c.mutation.AddAccountProxyPoolIDs(ids...)
	return _c
}

// AddAccountProxyPool adds the "account_proxy_pool" edges to the AccountProxy entity.
func (_c *ProxyCreate) AddAccountProxyPool(v ...*AccountProxy) *ProxyCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddAccountProxyPoolIDs(ids...)
}
...
func (_c *ProxyCreate) AddPooledAccountIDs(ids ...int64) *ProxyCreate {
	_c.mutation.AddPooledAccountIDs(ids...)
	return _c
}

// AddPooledAccounts adds the "pooled_accounts" edges to the Account entity.
func (_c *ProxyCreate) AddPooledAccounts(v ...*Account) *ProxyCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddPooledAccountIDs(ids...)
}
...
func (_c *ProxyCreate) createSpec() (*Proxy, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.AccountProxyPoolIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
	...
	if nodes := _c.mutation.PooledAccountsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
