...
// QueryProxyPool chains the current query on the "proxy_pool" edge.
func (_q *AccountQuery) QueryProxyPool() *ProxyQuery {
	query := (&ProxyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withProxyPool:      _q.withProxyPool.Clone(),
...
// WithProxyPool tells the query-builder to eager-load the nodes that are connected to
// the "proxy_pool" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *AccountQuery) WithProxyPool(opts ...func(*ProxyQuery)) *AccountQuery {
	query := (&ProxyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withProxyPool = query
	return _q
}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withProxyPool; query != nil {
		if err := _q.loadProxyPool(ctx, query, nodes,
			func(n *Account) { n.Edges.ProxyPool = []*Proxy{} },
			func(n *Account, e *Proxy) { n.Edges.ProxyPool = append(n.Edges.ProxyPool, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadProxyPool(ctx context.Context, query *ProxyQuery, nodes []*Account, init func(*Account), assign func(*Account, *Proxy)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Account)
	nids := make(map[int64]map[*Account]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.ProxyPoolTable)
		s.Join(joinT).On(s.C(proxy.FieldID), joinT.C(account.ProxyPoolPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.ProxyPoolPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.ProxyPoolPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "proxy_pool" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
