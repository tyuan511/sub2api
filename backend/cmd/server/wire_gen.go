...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	schedulerSnapshotService := service.ProvideSchedulerSnapshotService(schedulerCache, schedulerOutboxRepository, accountRepository, groupRepository, concurrencyCache, configConfig)
	...
	channelMonitorGatewayForwarder := service.NewChannelMonitorGatewayForwarder(gatewayService, openAIGatewayService, geminiMessagesCompatService)
	bazaarLinkProbeTaskRepository := repository.NewBazaarLinkProbeTaskRepository(client, db)
	bazaarLinkProbeReader := service.ProvideBazaarLinkProbeReader(bazaarLinkProbeTaskRepository)
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, usageLogRepository, secretEncryptor, settingService, channelMonitorGatewayForwarder, bazaarLinkProbeReader)
	channelMonitorUserHandler := handler.NewChannelMonitorUserHandler(channelMonitorService, settingService)
	channelMonitorV2Repository := repository.NewChannelMonitorV2Repository(db)
	channelMonitorV2ProbeReader := service.ProvideChannelMonitorV2ProbeReader(channelMonitorService)
	channelMonitorV2Service := service.ProvideChannelMonitorV2Service(channelMonitorV2Repository, settingService, channelMonitorV2ProbeReader)
	...
	bazaarLinkProbeService := service.ProvideBazaarLinkProbeService(channelMonitorRepository, secretEncryptor, bazaarLinkProbeTaskRepository, settingService)
	...
	supportAttachmentStore, err := repository.ProvideSupportAttachmentStore(configConfig)
	if err != nil {
		return nil, err
	}
	...
	handlerSupportHandler := handler.NewSupportHandler(supportService, supportTelegramService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, imageStudioHandler, batchImageHandler, handlerSupportHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
