...
func (r *channelMonitorRepository) InsertHistoryBatch(ctx context.Context, rows []*service.ChannelMonitorHistoryRow) error {
	...
	for _, row := range rows {
		...
		if row.TokensPerSecond != nil {
			c = c.SetTokensPerSecond(*row.TokensPerSecond)
		}
...
func (r *channelMonitorRepository) ListLatestPerModel(ctx context.Context, monitorID int64) ([]*service.ChannelMonitorLatest, error) {
	const q = `
		SELECT DISTINCT ON (model)
		    model, status, latency_ms, first_token_ms, tokens_per_second, ping_latency_ms, checked_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&l.Model, &l.Status, &latency, &firstToken, &tokensPerSecond, &ping, &l.CheckedAt); err != nil {
			return nil, fmt.Errorf("scan latest row: %w", err)
		}
		assignNullInt(&l.LatencyMs, latency)
		assignNullInt(&l.FirstTokenMs, firstToken)
		assignNullFloat(&l.TokensPerSecond, tokensPerSecond)
...
// assignNullFloat 把 sql.NullFloat64 解包到 *float64 指针目标。
...
func (r *channelMonitorRepository) ListLatestForMonitorIDs(ctx context.Context, ids []int64) (map[int64][]*service.ChannelMonitorLatest, error) {
	...
	const q = `
		SELECT DISTINCT ON (monitor_id, model)
		    monitor_id, model, status, latency_ms, first_token_ms, tokens_per_second, ping_latency_ms, checked_at, quota
		FROM channel_monitor_histories
	...
	for rows.Next() {
		...
		var tokensPerSecond sql.NullFloat64
		var quota []byte
		if err := rows.Scan(&monitorID, &l.Model, &l.Status, &latency, &firstToken, &tokensPerSecond, &ping, &l.CheckedAt, &quota); err != nil {
...
func (r *channelMonitorRepository) ListRecentHistoryForMonitors(
	...
	const q = `
		...
		SELECT monitor_id, status, latency_ms, first_token_ms, tokens_per_second, ping_latency_ms, checked_at
		FROM ranked
		WHERE rn <= $3
	...
	for rows.Next() {
		...
		var latency, firstToken, ping sql.NullInt64
		var tokensPerSecond sql.NullFloat64
		if err := rows.Scan(&monitorID, &entry.Status, &latency, &firstToken, &tokensPerSecond, &ping, &entry.CheckedAt); err != nil {
		...
		assignNullFloat(&entry.TokensPerSecond, tokensPerSecond)
