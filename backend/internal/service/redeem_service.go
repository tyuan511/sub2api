...
//
// Affiliate rebate and redeem activity notification are suppressed here because
// payment fulfillment accrues the rebate itself (applyAffiliateRebateForOrder)
// and emits its own recharge notification (notifyPaymentActivity).
func (s *RedeemService) redeemForPaymentFulfillment(ctx context.Context, userID int64, code string) (*RedeemCode, error) {
	return s.redeem(ContextSkipRedeemNotification(ContextSkipRedeemAffiliate(ctx)), userID, code, bypassRedeemRateLimit)
}
