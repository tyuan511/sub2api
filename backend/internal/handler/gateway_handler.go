...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if groups := apiKeyRouteGroupsForModels(c, apiKey); len(groups) > 1 {
		...
		if custom {
			writeAllowlistedModelsList(c, listPlatform, availableModels)
			return
		}
...
func (h *GatewayHandler) RouteGroupSupportsModel(ctx context.Context, group *service.Group, model string) bool {
	...
	return modelListAllowsModel(h.advertisedModelIDsForGroup(ctx, group, ""), model)
}

// modelListAllowsModel 报告 model 是否出现在可用模型/映射集合中。
// 与上游 ModelAllowlist.FilterForListing 的条目语义一致：精确匹配、后缀通配，
// 以及 Claude -thinking 宽容归一化；不做前缀/模糊匹配。
func modelListAllowsModel(availablePatterns []string, model string) bool {
