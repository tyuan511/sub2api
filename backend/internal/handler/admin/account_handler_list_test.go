...
func TestAccountHandlerListLiteUsesCompactDTOAndETag(t *testing.T) {
	...
	adminSvc.accounts = []service.Account{{
		...
		Proxies:       []service.AccountProxy{{AccountID: 501, ProxyID: 12, Concurrency: 3, SortOrder: 1}},
	...
	pool, ok := liteItem["proxies"].([]any)
	require.True(t, ok)
	require.Len(t, pool, 1)
	require.Equal(t, float64(12), pool[0].(map[string]any)["proxy_id"])
	require.Equal(t, float64(3), pool[0].(map[string]any)["concurrency"])
	...
	require.Equal(t, fullPayload.Data.Items[0]["proxies"], liteItem["proxies"])
}
