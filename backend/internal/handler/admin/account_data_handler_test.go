...
type dataAccount struct {
	...
	ProxyKey    *string            `json:"proxy_key"`
	ProxyPool   []dataAccountProxy `json:"proxy_pool"`
...
func TestExportDataWithoutProxies(t *testing.T) {
	...
	require.Empty(t, resp.Data.Accounts[0].ProxyPool)
}

func TestExportDataIncludesAccountProxyPool(t *testing.T) {
	router, adminSvc := setupAccountDataRouter()

	firstProxyID := int64(11)
	secondProxyID := int64(12)
	adminSvc.proxies = []service.Proxy{
		{ID: firstProxyID, Name: "proxy-a", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: service.StatusActive},
		{ID: secondProxyID, Name: "proxy-b", Protocol: "http", Host: "127.0.0.2", Port: 8080, Status: service.StatusActive},
	}
	adminSvc.accounts = []service.Account{
		{
			ID:          21,
			...
			ProxyPool: []service.AccountProxyConfig{
				{ProxyID: firstProxyID, Concurrency: 2},
				{ProxyID: secondProxyID, Concurrency: 5},
			},
			Concurrency: 7,
		},
	}
	...
	require.Len(t, resp.Data.Accounts, 1)
	require.Equal(t, "http|127.0.0.1|8080||", *resp.Data.Accounts[0].ProxyKey)
	require.Equal(t, []dataAccountProxy{
		{ProxyKey: "http|127.0.0.1|8080||", Concurrency: 2},
		{ProxyKey: "http|127.0.0.2|8080||", Concurrency: 5},
	}, resp.Data.Accounts[0].ProxyPool)
}
...
func TestImportDataRestoresAccountProxyPool(t *testing.T) {
	router, adminSvc := setupAccountDataRouter()
	adminSvc.proxies = []service.Proxy{
		{ID: 1, Name: "proxy-a", Protocol: "http", Host: "127.0.0.1", Port: 8080, Status: service.StatusActive},
		{ID: 2, Name: "proxy-b", Protocol: "http", Host: "127.0.0.2", Port: 8080, Status: service.StatusActive},
	}
			...
			"accounts": []map[string]any{
				{
					...
					"proxy_pool": []map[string]any{
						{"proxy_key": "http|127.0.0.1|8080||", "concurrency": 2},
						{"proxy_key": "http|127.0.0.2|8080||", "concurrency": 5},
					},
					"concurrency": 7,
					"priority":    50,
				},
	...
	require.Equal(t, []service.AccountProxyConfig{
		{ProxyID: 1, Concurrency: 2, Position: 0},
		{ProxyID: 2, Concurrency: 5, Position: 1},
	}, adminSvc.createdAccounts[0].ProxyPool)
}
