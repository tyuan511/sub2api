...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    it('classifies all configured platforms and retains the complete table filter', async () => {
      ...
      // 本地分组列表按平台/名称排序（compareGroupOptions），故比较集合而非原始顺序。
      expect([...optionIds(wrapper)].sort((a, b) => a - b)).toEqual([3, 4, 5, 6])
      await chooseProvider(wrapper, 'other')
      expect([...optionIds(wrapper)].sort((a, b) => a - b)).toEqual([7, 8, 9, 10, 11])
    ...
    it('clears the previous group on provider change and submits only the newly selected group', async () => {
      ...
      expect(createKeyWithRequest).not.toHaveBeenCalled()
      expect(showError).toHaveBeenCalledWith('keys.groupRequired')

      await groupSelect(wrapper).vm.$emit('update:modelValue', 5)
      createKeyWithRequest.mockResolvedValue({ ...createApiKey(), group_id: 5 })
      await wrapper.get('#key-form').trigger('submit')
      await flushPromises()
      expect(createKeyWithRequest).toHaveBeenCalledOnce()
      expect(createKeyWithRequest.mock.calls[0][0]).toMatchObject({ name: 'My key', group_id: 5 })
    })
