export default {
  ...
  keys: {
    ...
    groupRequired: '请选择分组',
    routeGroupsLabel: '候选分组与顺序',
    selectRouteGroups: '添加候选分组',
    routeGroupsSelected: '已选择 {count}/{max} 个分组',
    routeGroupHint: '最多选择 {max} 个同平台、同计费类型分组；列表顺序就是故障转移顺序。',
    ...
    moveGroupDown: '下移分组',
    removeGroup: '移除分组',
    scheduleModeLabel: '调度方式',
    scheduleSequential: '顺序调度',
    scheduleSequentialDescription: '严格从上到下尝试，组内资源耗尽后再切换分组。',
    scheduleSmart: '智能调度',
    scheduleSmartDescription: '在所选分组内根据价格与稳定性动态排序。',
    smartPreferenceLabel: '智能调度倾向',
    ...
    preferenceSpeedDescription: '优先提高成功率并缩短首 Token 与整体耗时。',
    preferenceBalanced: '均衡',
    preferenceBalancedDescription: '在价格与稳定性之间各占一半。',
    smartSchedulingDisclosure: '滑条在价格（费率倍数 + 缓存命中）与稳定性（成功率 + 首 Token + 耗时）之间按 5% 一档分配权重。成功率只参与打分，不会把分组直接熔断。系统只在你选择的候选分组内持续优化；已有健康会话保持当前分组，新会话使用最新排序，最终按实际服务分组展示和计费。',
    smartStrategyVersion: '当前策略 {version} · 更新于 {time}',
    smartStrategyPending: '当前暂无有效评分快照，请求将按你的候选顺序调度。',
    ...
    clickToEditRouting: '点击编辑候选分组与调度策略',
