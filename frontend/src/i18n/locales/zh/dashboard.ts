export default {
  ...
  keys: {
    ...
    groupRequired: '请选择分组',
    routeGroupsLabel: '候选分组列表',
    routeGroupsHelpTitle: '一把密钥可以绑定多个分组',
    routeGroupsHelpFailover: '当前分组不可用时，会故障转移到下一个仍能处理该请求的分组。',
    routeGroupsHelpModels: '请求进入实际支持该模型的分组，因此一把密钥可用已选分组里的全部模型。',
    routeGroupsHelpBilling: '实际请求仍会路由到具体分组，并按该分组计费。',
    routeGroupsHelpSequential: '顺序转移：按列表从上到下尝试。',
    routeGroupsHelpSmart: '智能转移：按价格与稳定性动态决定先后。',
    selectRouteGroups: '添加候选分组',
    routeGroupsSelected: '已选择 {count}/{max} 个分组',
    routeGroupHint: '最多选择 {max} 个同计费类型分组，可跨平台；请求会进入实际支持该模型的分组。实际请求依然会被路由到具体分组，并按实际路由到的分组计费。拖动列表即可调整故障转移顺序。',
    routePrimaryGroup: '首选分组',
    routeFallbackGroup: '备用分组',
    routeGroupRate: '当前倍率 ×{rate}',
    routeGroupUnavailable: '不可用',
    routeGroupUnavailableReason: '该分组当前不可用于新增绑定',
    routeGroupLimitReached: '最多只能选择 {max} 个候选分组',
    routeGroupPlatformMismatch: '只能选择与首个分组相同的平台',
    routeGroupBillingMismatch: '只能选择与首个分组相同的计费类型',
    dragToReorder: '拖动调整顺序',
    removeGroup: '移除分组',
    scheduleModeLabel: '故障转移方式',
    scheduleSequential: '顺序转移',
    scheduleSequentialDescription: '严格按列表顺序转移，组内资源耗尽后再切换分组。',
    scheduleSmart: '智能转移',
    scheduleSmartDescription: '在所选分组内根据价格与稳定性动态决定转移顺序。',
    smartPreferenceLabel: '智能转移倾向',
    balanceRatio: '价格 {price}% · 稳定性 {stability}%',
    sliderPrice: '更省成本',
    sliderSpeed: '更快响应',
    sliderStability: '更稳定',
    resetBalance: '重置为均衡',
    presetLowest: '极致低价',
    presetLowestHint: '能便宜就便宜',
    presetLowestDesc: '几乎只看价格，适合对稳定性不敏感的场景。',
    presetThrifty: '能省则省',
    presetThriftyHint: '便宜为主，略看可用',
    presetThriftyDesc: '大部分权重给价格，仍留一点成功率兜底。',
    presetBalanced: '均衡',
    presetBalancedHint: '价格和稳定对半',
    presetBalancedDesc: '价格与成功率大致均衡，适合大多数日常使用。',
    presetRock: '稳定如老狗',
    presetRockHint: '先保证能用',
    presetRockDesc: '成功率占绝对主导，几乎不考虑价格。',
    weightReliability: '成功率',
    weightCapacity: '容量',
    weightPrice: '价格',
    weightSpeed: '耗时',
    weightTTFT: '首 Token',
    successThreshold: '成功率熔断门槛',
    successThresholdDescription: '低于此成功率，暂停使用该分组',
    thresholdFlexible: '50% · 更宽容',
    thresholdStrict: '95% · 更严格',
    resetThreshold: '重置为 80%',
    successThresholdHint: '每档 5%，样本足够后生效，仅影响此 Key。门槛越高，可用分组可能越少；全部不达标时不会自动降低门槛。恢复需经过冷却与探测。',
    preferencePrice: '价格优先',
    preferencePriceDescription: '优先降低每次成功的实际成本（费率倍数与缓存命中）。',
    preferenceSpeed: '稳定性优先',
    preferenceSpeedDescription: '优先提高成功率，并缩短首 Token。',
    preferenceBalanced: '均衡',
    preferenceBalancedDescription: '在价格与稳定性之间各占一半。',
    smartSchedulingDisclosure: '系统只在你选择的候选分组内排序；已有健康会话保持当前分组，最终按实际服务分组展示和计费。',
    smartStrategyVersion: '当前策略 {version} · 更新于 {time}',
    smartStrategyPending: '当前暂无有效评分快照，请求将按你的候选顺序转移。',
		routeCurrentRank: '当前排名 #{rank}',
		routeEffectiveRate: '缓存修正 ×{rate}',
		routeSuccessRate: '成功率 {rate}',
		routeTTFT: '首 Token {value}ms',
		routeDuration: '耗时 {value}ms',
		routeCacheHit: '缓存命中 {rate}',
		routePredictedShare: '预计命中 {rate}',
		routeConfidence: {
			low: '低置信度',
			medium: '中置信度',
			high: '高置信度'
		},
		estimatedRateTitle: '当前预估倍率',
		estimatedRateWindow: '窗口 {window}',
		estimatedRateSamples: '样本：输入 {input} / 输出 {output} Token',
		estimatedRateObservedShare: '落点占比：{window} 真实样本 {samples}（有效样本约 {effective}）',
		estimatedRateModeledShare: '落点占比：统计不足，使用重试顺序模型',
		estimatedRateDisclaimer: '该区间是基于当前合格候选、缓存结构和故障转移概率的预估，不是固定报价；最终按实际服务分组和实际用量结算。',
    clickToEditRouting: '点击编辑候选分组与转移策略',
    routeConfigConflict: '该密钥的路由配置已在其他页面更新，已重新加载最新版本，请再次编辑。',
    usage: '用量',
    today: '今日',
    total: '近30天',
    quota: '额度',
  ...
  usage: {
    ...
    totalTokens: '总 Token',
    cacheTotal: '缓存',
    cacheHitRateHint: '缓存读取 Token ÷（输入 Token + 缓存读取 Token + 缓存创建 Token）',
