    ...
    <transition name="support-widget">
      ...
      >
        <header class="support-chat-header flex h-14 shrink-0 items-center justify-between border-b border-gray-200 px-3.5 dark:border-dark-700">
          <h2 class="min-w-0 truncate text-sm font-semibold text-gray-900 dark:text-white">{{ t('common.support.title') }}</h2>
          <button class="flex h-8 w-8 items-center justify-center rounded-md text-gray-500 transition hover:bg-gray-100 hover:text-gray-900 dark:text-dark-400 dark:hover:bg-dark-800 dark:hover:text-white" :aria-label="t('common.support.close')" :title="t('common.close')" @click="open = false">
            <Icon name="x" size="sm" />
          </button>
        </header>

        <div ref="messageStream" class="support-message-stream min-h-0 flex-1 space-y-4 overflow-y-auto px-3 py-4">
          <div v-if="loading" class="flex h-full items-center justify-center text-sm text-gray-500 dark:text-dark-400">{{ t('common.support.loading') }}</div>
          <div v-else-if="!messages.length" class="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <p class="mt-3 text-sm font-medium text-gray-800 dark:text-dark-100">{{ t('common.support.emptyTitle') }}</p>
            <p class="mt-1 text-xs leading-5 text-gray-500 dark:text-dark-400">{{ t('common.support.emptyDescription') }}</p>
          </div>
          ...
          <article v-for="message in messages" :key="message.id" class="flex items-start gap-2" :class="message.sender_role === 'user' ? 'justify-end' : 'justify-start'">
            ...
            <div :class="message.sender_role === 'user' ? 'max-w-[88%] sm:max-w-[78%]' : 'max-w-[calc(100%_-_2.5rem)] sm:max-w-[78%]'">
              ...
              <div v-if="message.attachments?.length" class="mb-2 flex flex-col gap-2" :class="message.sender_role === 'user' ? 'items-end' : 'items-start'">
                <button v-for="attachment in message.attachments" :key="attachment.id" class="max-w-full cursor-zoom-in overflow-hidden rounded-md bg-gray-200 dark:bg-dark-800" :aria-label="t('common.support.previewAttachment', { name: attachment.original_name })" @click="openImage(attachment.id, attachment.original_name)">
                  ...
                  <span v-else class="flex h-28 w-40 items-center justify-center text-[11px] text-gray-500">{{ t('common.support.attachmentLoading') }}</span>
                </button>
              </div>
              <div
        ...
        <form class="support-composer flex h-[154px] shrink-0 flex-col border-t border-gray-200 dark:border-dark-700" @submit.prevent="sendMessage">
          <div class="flex h-10 shrink-0 items-center gap-1 px-3">
            ...
            <span v-if="files.length" class="min-w-0 truncate text-xs text-gray-500 dark:text-dark-400">{{ t('common.support.pendingImages', { count: files.length }) }}</span>
            <button v-if="files.length" type="button" class="flex h-7 w-7 items-center justify-center rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700 dark:hover:bg-dark-800 dark:hover:text-white" :title="t('common.support.clearImages')" :aria-label="t('common.support.clearImages')" @click="clearFiles">
              <Icon name="x" size="sm" />
            </button>
          </div>
          ...
          <div class="relative min-h-0 flex-1">
            <textarea
              ...
              :disabled="sending"
              :placeholder="t('common.support.inputPlaceholder')"
...
async function loadConversation(silent = false) {
  ...
  try {
    ...
    appStore.showError(error?.message || t('common.support.loadFailed'))
  } finally {
...
function selectFiles(event: Event) {
  ...
  if (selected.length > 2 || selected.some((file) => file.size > 3 * 1024 * 1024)) {
    appStore.showError(t('common.support.tooManyImages'))
    input.value = ''
    return
  }
  clearFiles()
...
async function sendMessage() {
  ...
  try {
    ...
    if (!current) {
      ticket.value = await createSupportTicket({ content: content || t('common.support.sentImage'), files: files.value })
    } else {
    ...
    appStore.showError(error?.message || t('common.support.sendFailed'))
  } finally {
    sending.value = false
    await nextTick()
